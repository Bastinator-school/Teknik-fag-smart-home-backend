...
func NewMQTTClient(opts MQTT_options, hub *ws_hub) *MQTTClient {
	m := &MQTTClient{
	...
	options.AddBroker(opts.broker_url)
	options.SetClientID(opts.client_id)
	options.SetUsername(opts.broker_user)
	options.SetPassword(opts.broker_pass)
	...
	options.SetOnConnectHandler(func(c mqtt.Client) {
		...
		topics := map[string]byte{
			...
			"home/livingroom/temp":                     1, //this subscribes to /home/living_room/(any light)/state
		}
		m.subscribe(topics)
	})
